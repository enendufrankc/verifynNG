...
import { InternalOnly } from '../../common/internal-only.decorator';
import { SmsPort, SMS_PORT } from './sms.port';
 ...
 *
 * Marked `@InternalOnly('sms:inbound')`; the bearer check below is the
 * temporary gate until E02 ships the real ApiKey guard.
 */
@ApiTags('verify')
...
export class VerifySmsController {
  ...
  private readonly fakeSmsKey: string;
  ...
  async verifySms(
    ...
    // --- 0. Authenticate (temporary until E14 ships real auth) ------------
    const auth = headers['authorization'];
    if (!auth || auth !== `Bearer ${this.fakeSmsKey}`) {
