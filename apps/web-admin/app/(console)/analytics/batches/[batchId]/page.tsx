...
export default function BatchAnalyticsPage() {
  ...
  const verdictsQuery = useQuery({
    queryKey: ['analytics', 'batch-verdicts', tenantId, batchId, range],
    queryFn: () => getVerdicts(range, { batchId }),
    enabled: !!tenantId,
  });
  const geoQuery = useQuery({
    queryKey: ['analytics', 'batch-geo', tenantId, batchId, range],
    queryFn: () => getGeo(range, 'country', { batchId }),
    enabled: !!tenantId,
  });
  ...
  return (
    <div className="space-y-s6">
      <PageHeader
        title={`Batch ${batchId}`}
        description="Scan activity for this batch."
        actions={
