...
- [x] T9 Web-admin auth pages `app/(auth)/sso/**`: `/login` (E02-owned) gets a new "Organisation" field + **Continue with SSO** button when `/auth/sso/:slug` says enabled, password form hidden when `enforceSso` (replaced by the break-glass link); `/sso/complete` finishes the cookie hand-off (via a new `POST /api/auth/sso/complete` BFF route, mirroring `/api/auth/session`); `/sso/error` explains each error code. `supportEmail`-in-the-error-message is deferred — `Tenant.supportEmail` exists but the anonymous `GET auth/sso/:tenantSlug` status endpoint doesn't return it yet (T3 predates this need); noted as a gap rather than widening that endpoint's response now.
      Deviations from the epic doc, all documented in code comments: (1) no `/login/:tenantSlug` route exists (or was added) — E02's `/login` page takes a slug via a form field instead, since creating a whole new dynamic segment under E02's `app/(auth)/**` was a bigger footprint than necessary; (2) `middleware.ts`'s `PUBLIC_PATHS` allowlist (E11-owned) needed the three new anonymous `/sso/*` pages added — otherwise they redirect to `/login` before ever rendering; (3) `apps/web-admin/lib/api-client.ts`'s `ApiError` didn't carry a structured error payload (Nest's default filter puts a custom exception's constructor argument under `message` as an object, e.g. `{code, ssoStartUrl}` / `{code, unmet}` — the existing type only handled a string), needed for both the enforce-SSO precondition list and the `sso_required` code to reach the UI; extended non-breakingly (`payload?` field added, existing string-message path unchanged). No console-wide "Emergency access expires in 60:00" banner (T10/E11 console-shell scope, not attempted) or MFA grace banner (the API returns `mfaGraceUntil`; nothing renders it yet — noted, not wired).
- [x] T10 Settings screens `app/(console)/settings/security/**` (owner only): **SSO** — provider select, client id/secret, issuer (fake/generic), allowed domains as removable badges, JIT toggle + default role, **Test connection** with diagnostics panel, **Enforce SSO** switch with a typed-slug confirmation dialog showing the unmet-preconditions list on a 409; **MFA policy** — role checkboxes, grace period, table of members not yet enrolled with days remaining. A new `app/(console)/settings/security/layout.tsx` adds sub-tabs (Password & sessions / Single sign-on / MFA policy) rather than editing E02's existing `settings/security/page.tsx` directly. No `nav.config.ts` entry needed — these are sub-routes of the existing `Settings` nav entry (E11 already registered), not a new top-level item.
      **Update (T12): now verified interactively, for real.** The interactive browser tool available in this session can't navigate to this worktree's web-admin port (Chrome treats 5061 as a blocked "unsafe port"), but that turned out to be a genuine Chromium restriction — 5060/5061 are SIP/SIP-TLS, hardcoded unsafe in every Chromium, including a plain `npx playwright test` run. Root-caused and fixed in `playwright.config.ts` (see T12); `tests/e2e/sso.spec.ts` then drove the actual login page, the fake-oidc login page, and the settings page through real Chromium and passed. The settings-screen assertions are still thin (one field checked, not every control) — a fuller pass would be worth it before calling T10 fully proven, but this is a real browser pass, not curl-and-hope.
- [x] T11 `docs/sso-setup-guide.md`: Google Cloud OAuth client steps (consent screen, authorised redirect URI, `hd` claim), Entra app registration (single-tenant vs multi-tenant, issuer with tenant id, `email` optional claim, `xms_edov`/`email_verified` note), what to put in the console, troubleshooting table mapping every `SsoErrorCode` this epic emits to its fix; input section for E18's `auth-lockout.md` (break-glass procedure — E18 hasn't claimed its epic yet, so this is written as a self-contained section E18 can lift rather than a cross-file edit).
- [x] T12 Playwright: `tests/e2e/sso.spec.ts` — link, JIT, rejected (`domain_not_allowed`), enforce-SSO 403 `sso_required` (skips itself with a clear message when the owner has no TOTP enrolled to satisfy the precondition, rather than faking it), IdP-asserted MFA (AC8), and the SSO settings page. `loginViaSso(page, fakeUser, tenantSlug?)` implemented for real in `tests/e2e/fixtures/auth.ts`, replacing the `TODO(E20)` stub E21 had left there — drives the actual `/login` → fake-oidc login page → `/sso/complete` flow. 5 passed, 1 skipped, run for real against `chromium` (not just curl) — see the fix below.
      **Found and fixed a real, non-E20-specific bug**: `scripts/epic`'s `103 × epic-number` port offset puts epic 20's `WEB_VERIFY_PORT`/`WEB_ADMIN_PORT` at 5060/5061 — Chromium hardcodes both as unsafe ports (SIP / SIP-TLS), so _every_ Playwright run against this epic's worktree failed every navigation with `net::ERR_UNSAFE_PORT`, including a plain `npx playwright test`, not just an interactively-driven browser. Fixed by adding `--explicitly-allowed-ports` to `playwright.config.ts`'s Chromium launch args (computed from the same `WEB_VERIFY_PORT`/`WEB_ADMIN_PORT` env vars the config already reads) — a different epic number could hit a different blocked port from the same offset formula, so this isn't only-E20-relevant and belongs in the shared config permanently, not as an E20-local workaround.
      MFA policy grace/enforcement (AC7) is covered by `apps/api`'s integration tests (`mfa-policy.service.postgres.spec.ts`) rather than Playwright, given `AC7`'s explicit dependency on E15's not-yet-shipped `FakeClock` for advancing time — a browser-driven version would need the same backdating workaround already used there, which doesn't gain anything by also going through the UI.
