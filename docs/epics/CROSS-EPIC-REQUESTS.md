...
- [ ] One-line call to E10's `productPageRewrite(request)` (`apps/web-verify/lib/product-page/resolve-tenant.ts`) from `middleware.ts`, same shape as T7's `registerTier1Renderer` designated hook — if it returns non-null, return that response before the rest of `middleware.ts`'s own logic. Lets a tenant's custom domain (`ivoryglow.localhost:3000/turmeric-curcumin`, resolved via the `PAGE_DOMAIN_STUB` readiness stub until E03 ships `GET /v1/tenants/by-domain/:host`) rewrite to `/p/ivoryglow/turmeric-curcumin` (T9 — routing readiness only, no DNS/TLS).
