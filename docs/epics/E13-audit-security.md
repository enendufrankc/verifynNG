...
- [x] T6 `packages/config/src/security/`: `buildCsp({ nonce, apiOrigin, extraConnect? })`, `SECURITY_HEADERS`, `corsAllowlist(app: 'admin'|'verify'|'api')` from env `CORS_ORIGINS_ADMIN`, `CORS_ORIGINS_VERIFY`. Nest: helmet with the shared header set, HSTS, CORS allowlist. Next: `middleware.ts` in both apps generating a per-request nonce, setting CSP (report-only via `CSP_REPORT_ONLY=true`), passing the nonce via `x-nonce` header for `<Script nonce>`. `docker/compose.yml`'s `CORS_ORIGINS_ADMIN`/`CORS_ORIGINS_VERIFY` now derive from the same per-worktree `WEB_ADMIN_PORT`/`WEB_VERIFY_PORT` vars `scripts/epic` writes, so the allowlist is correct in every worktree, not just the default-port one.
- [x] T7 `SecretsModule`: `SecretsPort`, `EnvFileSecrets`, `SecretsKeyRing` (parses `CORE_KEYS_JSON`, falls back to E01's `CORE_KEYS`/`CORE_ACTIVE_KID` env for backwards compatibility), bound as the app-wide `KeyRing` provider. `docs/security/secrets.md` written. Fixed along the way: `SecretsKeyRing`'s factory was sourcing `CORE_KEYS_JSON` via `ConfigService`, which always sees the zod-defaulted value (Nest's `validate` option writes it into `process.env`) — a rotated file value could never actually take effect. Now reads via `EnvFileSecrets.getFromFile()` first.
- [x] T8 `pnpm secrets:rotate-core-key [--file docker/secrets/local.env] [--kid k3]`: generates 32 random bytes, appends to `CORE_KEYS_JSON.keys`, flips `active`, refuses to delete any kid, prints a redacted diff. `docs/security/key-rotation-runbook.md` written: rotate → restart → confirm new mints carry new kid → confirm pre-rotation codes still verify → never retire a kid while any printed batch references it → retirement checklist. `docker/compose.yml` mounts `./secrets` (the directory, not the file — so a fresh `docker compose up` before anyone has ever rotated doesn't bind-mount a missing file into an empty directory) into the api container read-only.
...
- [x] T10 CI security: `.github/workflows/security.yml` (`pnpm-audit`, `gitleaks`, `codeql` jobs) + `.gitleaks.toml` (allowlist for `packages/core/test/fixtures`) + `.github/dependabot.yml` (npm weekly grouped, github-actions monthly, docker monthly) + `docs/security/ci-gates.md`. **Caveat**: GitHub Actions is account-wide disabled right now (per `AGENTS.md`'s temporary note), so none of these jobs have actually run yet — can't demonstrate AC7 against a live run until that's lifted. `pnpm audit --audit-level=high` currently fails locally against 4 pre-existing high-severity transitive advisories (`postcss` via `next`, `deepmerge-ts` via `prisma`) — documented as a known baseline in `ci-gates.md` rather than worked around.
- [x] T11 `SECURITY.md` (reporting channel, supported versions, disclosure SLA, scope) and `docs/security/threat-model.md` (STRIDE over: verify endpoint, mint path, manifest delivery, admin console, audit log itself; honest limits from mental-model §5; encryption-at-rest statement for Postgres/MinIO volumes; incident response outline with NDPR 72 h / UK GDPR notification steps). Both linked from `README.md`.
- [x] T12 Wire-up PR: one-line imports of `AuditModule`, `QuotaModule`, `SecretsModule` in `AppModule`; env section "E13" in `packages/config` with compose defaults (`CORE_KEYS_JSON` dev value, `CORS_ORIGINS_*`, `CSP_REPORT_ONLY=true`, `SECRETS_FILE`). No separate `pnpm secrets:init` — `pnpm secrets:rotate-core-key` (T8) already bootstraps `docker/secrets/local.env` from scratch when it's missing, and the app works fine with no file at all (falls back to the schema default), so a dedicated init step wasn't needed.
...
- [x] AC3 Verified on this worktree's ports: `curl -I` against both web-admin and web-verify includes `Content-Security-Policy-Report-Only` with a `'nonce-…'` value that changes per request, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-Content-Type-Options: nosniff`; `curl -I .../health` shows helmet's header set; `curl -H 'Origin: http://evil.example'` returns no `Access-Control-Allow-Origin`, while the real web-admin/web-verify origins do.
- [x] AC4 `pnpm secrets:rotate-core-key` → `docker/secrets/local.env` gains a new kid and `active` flips; `docker compose restart api`; `curl .../v1/_dev/keyring` reports the new active kid and still lists the old one; a code minted before rotation (via `generateCode()` against the pre-rotation active kid) still passes `GET /v1/_dev/keyring/verify?code=...` after rotation. Verified on this worktree's ports.
...
- [ ] AC7 Not demonstrated — GitHub Actions is account-wide disabled (per `AGENTS.md`'s temporary note), so no workflow run exists to point at yet. The jobs (`security.yml`'s `pnpm-audit`/`gitleaks`/`codeql`) and `.gitleaks.toml`/`.github/dependabot.yml` are in place; re-verify this AC once Actions is restored. Note `pnpm-audit` is currently red against `main`'s own dependency tree (pre-existing transitive advisories, see `docs/security/ci-gates.md`) — that needs addressing before this AC can go green even once Actions is back.
- [x] AC8 `SECURITY.md` and `docs/security/threat-model.md` exist, are linked from `README.md`, and the threat model lists every mitigation with the epic that implements it.
