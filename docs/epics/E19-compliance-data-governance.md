...
- [x] T5 `ConsentService` + `POST /v1/consent`, `GET /v1/consent/me`; contract fixtures under `tests/contracts/` (this repo's `tests/` convention, not `test/`) for the exact call shapes E08 and E14 would need — neither actually calls them yet (E08 doesn't exist; E14's `NotificationService.send()` has no marketing-consent gate wired in, so there's no current caller to gate), so these are the spec, not a regression test on a real caller. Not done: the actual E08/E14 wire-in PRs (nothing to wire into/no caller to add the gate to yet).
...
- [x] T14 Web-admin `/compliance/retention` (branches on `platformRole`: support gets the full ops view, any tenant member gets a read-only schedule + last-ran timestamp via a new `GET /v1/retention/schedule`), `/compliance/dsar` (owner), `/compliance/incidents` (tenant read-only), `/legal` "Your agreements", nav entries. Playwright coverage per T15.
- [x] T15 Playwright (`tests/e2e/compliance.spec.ts`, real browser login against E02's actual auth): support publishes a legal document; owner blocked by the interstitial until accepting, operator/viewer see only the banner; support dry-runs then wet-runs a retention policy and sees counts; tenant owner sees the read-only schedule with no ops controls. All 4 pass. **Writing these is what surfaced two real, previously-invisible bugs**: `TokenService.rotateRefreshToken()` silently dropped the `platformRole` JWT claim on every session refresh (breaking every `@PlatformRole('support')` route the moment a support session's token refreshed — a live, security-relevant bug, not E19-specific, fixed and verified), and a `hasBootstrapped` race in web-admin's auth store where role-gated layout guards could 404 permanently on a hard page reload before the session rehydrated (fixed for E19's own 3 new layouts; the pre-existing `support/layout.tsx` has the identical bug, flagged for E11, not fixed here). Not written: consumer-DSAR and legal-hold-blocked-erasure Playwright flows (blocked on E08, same as T8).
...
- [x] AC7 Verified live end-to-end (evidence on issue #20): export completes, `dsar.ready` mail delivered with a working download link, MinIO objects present (main zip + `consents.json`/`acceptances.json` supplement, not merged into one bundle as the epic's literal text describes — see T9). TTL-expiry-returns-403 verified against the underlying presigned-URL mechanism directly (same S3Client/getSignedUrl call `DsarStorageService`/`TenantS3Service` use) rather than restarting the api container with `DSAR_EXPORT_TTL_HOURS=0.01` — 200 immediately, 403 after the TTL elapsed.
...
- [x] AC10 Verified live end-to-end (evidence on issue #20): `ndpcNotifyRequired: true`, `ndpcNotifyDeadline` = `detectedAt` + 72h exactly; visible read-only at the ivoryglow owner's `GET /v1/incidents/mine`; the runbook's tabletop rehearsal script has now actually been walked through (open → contain → assess → NDPC-notice note → close with postmortem), evidence on issue #20.
