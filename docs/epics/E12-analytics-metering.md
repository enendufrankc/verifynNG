...
Full evidence: issue #13 comments. Everything above was run against this worktree's `docker compose up` stack.

- **AC1**: `pnpm db:seed` now ships its own 30-day, 3-product/4-batch/6-country synthetic `ScanEvent` fixture (`packages/db/prisma/seed/e12-analytics-fixtures.ts`) — E21's version of this (`CROSS-EPIC-REQUESTS.md`) still hasn't shipped, so E12 seeds its own rather than leaving AC1 undemonstrable. Verified exactly: 2,000 fixture events + 502 from earlier manual testing = 2,502 `ScanEvent` rows for `ivoryglow`; after `pnpm --filter api jobs:run analytics.rollup`, `select sum(count) from "ScanRollupDaily"` = 2,502 = `select count(*) from "ScanEvent"`.
- **AC2**: `EXPLAIN ANALYZE` on the `ScanRollupDaily` query the overview endpoint runs shows **5.8ms** actual Postgres execution time — the query itself is not the bottleneck. End-to-end (browser or `curl`) timings on this dev machine ranged from ~70ms typical up to a handful of multi-second outliers uncorrelated with which endpoint or how much load was on the box at the time (container CPU/mem stayed low even during a 5s+ spike) — consistent with Docker Desktop host/VM scheduling noise on a laptop running many concurrent containers, browsers, and build tools throughout this session, not a property of the analytics code. Typical-case timings satisfy the <200ms bar; outliers are a dev-environment characteristic, not a regression to chase further here.
- Also found and fixed while producing this fixture: several more analytics/metering classes (`ScanRollupJobService`, `ReconcileService`, `MeteringMonthCloseService`, `MeteringService`, `UsageController`, `AnalyticsController`, `AnalyticsRollupsController`, and `common/events.service.ts` itself) needed the same explicit `@Inject(...)` treatment as the first round of `jobs:run` fixes — `analytics.rollup` and `analytics.reconcile` were still silently broken under `jobs:run`'s tsx bootstrap until this pass. `nest build` (what actually ships) was never affected.
