...
- [x] T7 Share-safe pages: after hydration `history.replaceState` rewrites `/v/<full code>` → `/v/<redacted>`, the full code is never placed in any DOM node/attribute/script (server passes only the redacted form to the client — verified via `curl`); `robots.ts` disallows `/v/`. OG image generation (`opengraph-image.tsx`) not built yet — OG tags are text-only for now, still code-free.
