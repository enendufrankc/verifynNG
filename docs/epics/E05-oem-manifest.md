...
- [ ] AC3 `curl "localhost:4000/v1/oem/deliveries/<id>/manifest?token=<token>"` six times → the sixth returns `410 max_downloads_reached`; after `POST /tenants/<tenantId>/deliveries/<id>/resend` the old token returns `410 token_revoked` and the new one works.
- [ ] AC4 `pnpm oem:receipt tools/oem-receipt/fixtures/printed-ok.csv` prints `codeCount`, one watermark and a `receiptHash`; `--submit http://localhost:4000/v1/oem/deliveries/<id>/receipt --token <oem jwt>` (note: the full receipt URL, delivery id included — `--submit` posts exactly the JSON summary to whatever URL you give it) against a _real_ delivered batch's own printed CSV → batch becomes `printed`; `GET /v1/audit?action=batch.receipt` shows the audited row with actor type `oem`. The two fixture CSVs are synthetic (real codes, but not from any actual delivery) — they exercise the CLI's own parsing/hashing, not a live `--submit` match.
- [ ] AC5 Same `--submit` flow with one printed code swapped for a code from a different batch's watermark → `matched: false, mismatchReason: 'watermark'`, batch stays `delivered`, `receipt.mismatch` email arrives in Mailpit for the owner and the tenant deliveries screen shows a red receipt badge (`tools/oem-receipt/fixtures/printed-swapped.csv` demonstrates the CLI-side detection of this shape — two distinct watermarks in the parsed set).
- [ ] AC6 In the OEM portal, submit the ship form with carrier `DHL`, tracking `1234567890` → batch `shipped`; `GET /tenants/<tenantId>/batches/<id>/deliveries` includes the delivery; attempting `POST /v1/oem/deliveries/<id>/ship` again returns `409 illegal_transition shipped→shipped`.
- [ ] AC7 Illegal transitions are rejected with `409` for every non-edge in the state machine (table-driven integration test), and `POST /tenants/<tenantId>/batches/<id>/close` as owner works from `shipped`; as `operator` it returns `403`.
- [ ] AC8 Cross-tenant: an `oem` user of tenant A listing `/v1/oem/deliveries` never sees tenant B rows, and fetching tenant B's delivery by id gets `404` (never confirming it exists) while a same-tenant, wrong-OEM fetch gets `403`. E02's `assertTenantIsolation()` harness only mints owner/operator/viewer fixtures and probes via a single owner-authenticated call, so this is a purpose-built OEM isolation test in E05 (`oem-manifest.integration.spec.ts`) rather than a literal extension of that harness.
