...
- [x] T12 `OemManifestModule` wired into `AppModule`; env section "E05" (`OEM_PORTAL_BASE_URL`, `DELIVERY_DEFAULT_EXPIRY_HOURS`, `DELIVERY_DEFAULT_MAX_DOWNLOADS`). Playwright: `tests/e2e/oem-manifest.spec.ts` drives real login directly (the shared `loginAs()` fixture in `tests/e2e/fixtures/auth.ts` is still E02/E11's unimplemented stub — a bare `page.goto('/')` — so this doesn't depend on it); covers AC1 (deliver via the console UI, Mailpit assertion) and the OEM-portal landing/isolation UX. Login now routes an `oem`-role user to `/oem/deliveries` instead of the tenant console root.
