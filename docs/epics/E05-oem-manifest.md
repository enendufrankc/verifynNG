...
- [x] T1 Cross-epic: commented on E02's issue (#3) and E04's issue (#5) rather than opening separate PRs against already-merged/closed epics — added `oem` to `TenantRole` directly (additive migration) and confirmed no other E02 change is needed (`RolesGuard`'s hierarchy map already falls back to treating an unrecognised role as its own singleton). No `oemId` JWT claim: `OemScopeGuard` resolves `OemUser` from the DB per-request instead, which is the fallback the spec itself calls for. `OemManifestModule` scaffolded.
- [x] T2 `BatchLifecycleService` — transition table, `batch.status.changed` event, unit tests for every legal/illegal pair. Writes `Batch.status` directly via the shared Prisma client (E04 never published a `BatchService.setStatus`).
- [x] T3 Migration `E05_oem_delivery`: `OemUser`, `ManifestDelivery`, `ManifestDownload`, `PrintReceipt`, `Shipment`, `DeliveryStatus`, `TenantRole.oem`, `Batch.expectedShipDate`.
- [x] T4 `DeliveryService.deliver`/`revoke`/`resend` — re-signs E04's manifest with a `delivery` block, AES-256-GCM envelope matching E04's, 32-byte token (hash stored), `expectedShipDate` written on `Batch`, `manifest.delivered` emailed directly to OEM users (tokenised OEM-portal link) and tenant owners (plain dashboard link).
- [x] T5 OEM download routes — constant-time token check, expiry/max-downloads/revoked handling (410s), `manifest_downloads_per_hour` quota (self-registered in `OemManifestModule.onModuleInit` so it works under `Test.createTestingModule` too, not just `main.ts`'s bootstrap), `ManifestDownload` row + `AuditService.record` with actor type `oem`, `manifest.downloaded` event. Artwork redirect via E04's `ExportsService.getSignedUrl`.
- [x] T6 `ReceiptService.verify` — hash/count/watermark comparison against the batch's _stored_ watermark (not re-derived from the manifest's `tenant` field, which is E04's tenant _id_, not the slug `deriveBatchWatermark` needs — a real bug this epic's own integration test caught before it shipped). Idempotent resubmission, `receipt.mismatch` → E14.
- [x] T7 Shipment + owner `close`, both state-machine-validated before the write (a repeat `ship` 409s via `BatchLifecycleService`, never a raw unique-constraint error).
- [x] T8 OEM user management (`OemUserService.invite/list/remove`) — invite via E14's `password.reset` flow (no `invite: true` template variant added; the existing template's wording already fits). Seed: "Guangzhou Pack Co." + `oem@guangzhou-pack.test` + a genuine 20-unit minted batch with a real encrypted manifest in MinIO.
- [x] T9 web-admin tenant side `(console)/deliveries/` — list, "Deliver batch" dialog, detail (download log, receipts, revoke/resend). Nav entry "Deliveries". (Needed a tenant-wide `GET /tenants/:tenantId/deliveries` the original per-batch-only route table didn't have — added to `DeliveryService`/`DeliveryActionsController`.)
- [x] T10 web-admin OEM portal — real URL is `app/(oem)/oem/**` (a bare `(oem)/deliveries` route group would collide with the console's `(console)/deliveries` at the same `/deliveries` URL), separate layout with client-side role redirect, download/artwork/receipt/ship UI.
- [x] T11 `tools/oem-receipt` CLI — own CSV parser (no added dependency), pure `receipt-builder` module with unit tests, tsup single-file bundle, `docs/oem/receipt-cli.md` + `docs/oem/oem-portal-guide.md`. Added `tools/oem-receipt` to the pnpm workspace (it wasn't globbed by default) and a root `oem:receipt` script.
- [x] T12 `OemManifestModule` wired into `AppModule`; env section "E05" (`OEM_PORTAL_BASE_URL`, `DELIVERY_DEFAULT_EXPIRY_HOURS`, `DELIVERY_DEFAULT_MAX_DOWNLOADS`). Playwright: `tests/e2e/oem-manifest.spec.ts` covers AC1 (deliver via the console UI, Mailpit assertion) and the OEM-portal landing/isolation UX. `tests/e2e/fixtures/auth.ts`'s `loginAs('oem')` was still E02/E11's unimplemented stub (a bare `page.goto('/')`) — implemented it for real (drives the actual `/login` form against the default `pnpm db:seed` fixtures) since it had zero other callers to break; also fixed `tests/e2e/fixtures/mailpit.ts`'s `waitForEmail()`, which read Mailpit's `To: [{Name, Address}]` as if `To` were an array of plain strings. Login now routes an `oem`-role user to `/oem/deliveries` instead of the tenant console root.

## Acceptance criteria

- [x] AC1 Owner logs in for real (`tests/e2e/oem-manifest.spec.ts`, since `loginAs()` is still unimplemented) at `/deliveries`, delivers the seeded minted batch to "Guangzhou Pack Co." → row appears `delivered`; Mailpit shows `manifest.delivered` to `oem@guangzhou-pack.test` **and** the owner; `GET /tenants/ivoryglow/batches/<id>` reports `status: delivered` and `expectedShipDate` set. Verified both by the Playwright test and manually against `docker compose up`.
- [x] AC2 Following the email link as the OEM downloads a JSON whose `core.verifyManifest` passes with the active kid, `downloadCount` becomes 1 and the download log shows the IP; a second OEM user from another seeded OEM (Guiba OEM) requesting the same delivery gets `403`. Verified live against `docker compose up`.
- [x] AC3 `curl ".../v1/oem/deliveries/<id>/manifest?token=<token>"` past `maxDownloads` returns `410 max_downloads_reached`; after `POST /tenants/<tenantId>/deliveries/<id>/resend` the old token returns `410 token_revoked` and the new one works. Verified live.
- [x] AC4 `pnpm oem:receipt <printed.csv>` (built from a real downloaded manifest's own `tier2Code`s) prints `codeCount`, one watermark, a `receiptHash`; `--submit <full receipt URL> --token <oem jwt>` → batch becomes `printed`; `GET /v1/audit?action=batch.receipt` shows the audited row with actor type `oem`. Verified live end-to-end. The two committed fixture CSVs (`printed-ok.csv`/`printed-swapped.csv`) are synthetic — they exercise the CLI's own parsing/hashing/watermark-counting in its Vitest suite, not a live `--submit` match against a real batch (a fixture can't know a live delivery's actual codes).
- [x] AC5 Submitting a receipt with one code swapped for a different batch's watermark → `matched: false`, `watermarks: [<original>, <foreign>]`, batch stays `delivered`, `receipt.mismatch` email arrives for the owner. `mismatchReason` came back `'hash'` rather than `'watermark'` in this run: `compareReceipt` checks hash first, and swapping any single code also changes the whole-set hash, so `hash` legitimately fires first — `mismatchDetail: { hashMatch:false, countMatch:true, watermarkMatch:false }` still surfaces the watermark evidence. Verified live; the tenant deliveries screen shows the red mismatch badge (`receiptBadge()` in `(console)/deliveries/page.tsx`).
- [x] AC6 Submitting the ship form (carrier `DHL`, tracking `1234567890`) → batch `shipped`; `GET /tenants/<tenantId>/batches/<id>/deliveries` includes the delivery; a second `POST /v1/oem/deliveries/<id>/ship` returns `409 illegal_transition shipped→shipped`. Verified live.
- [x] AC7 Illegal transitions are rejected with `409` for every non-edge in the state machine (table-driven unit test on `BatchLifecycleService` + integration coverage), and `POST /tenants/<tenantId>/batches/<id>/close` as owner works from `shipped`; as `operator` it returns `403`. Verified live and by test.
- [x] AC8 Cross-tenant: an `oem` user of tenant A listing `/v1/oem/deliveries` never sees tenant B rows, and fetching tenant B's delivery by id gets `404` (never confirming it exists) while a same-tenant, wrong-OEM fetch gets `403`. E02's `assertTenantIsolation()` harness only mints owner/operator/viewer fixtures and probes via a single owner-authenticated call, so this is a purpose-built OEM isolation test in E05 (`oem-manifest.integration.spec.ts`) rather than a literal extension of that harness. Verified by that integration test (real Postgres/MinIO/HTTP).
