...
- **GTIN** — what the product *is* (same for every unit)
- **Batch/Lot** — when and where it was made
- **Serial** — *this specific unit* (crypto-random, unguessable, never sequential)
...
1. **The serial alone proves nothing — the event history is the proof.** Every legitimate supply-chain node logs scan events (EPCIS). A serial seen in Lagos, Kano, and Accra within a week is counterfeit evidence *regardless* of code validity.
...
- Encodes: product identity + batch (+ optionally unit serial, but *without claims semantics*)
...
- **Unforgeable**: checksum means validity is checkable *before* any DB hit; forging requires the server secret.
...
- A determined counterfeiter can copy one real Tier-2 code onto some fakes. The system *detects* this via duplicate scan history; it cannot *prevent* it. Detection is the product.
...
*Companion artifacts: `ivory-glow-page/` (Tier-1 product page prototype), `IVORY-GOLD/` (label PDFs + trademark letter — tenant #1 source of truth).*
