...
@theme {
  ...
  /* ── shadows ─────────────────────────────────────────────────────────────── */
  --shadow-sm: 0 1px 2px rgba(19, 23, 32, 0.05);
  --shadow-md:
    0 4px 16px rgba(19, 23, 32, 0.07), 0 1px 3px rgba(19, 23, 32, 0.04);
  --shadow-lg:
    0 18px 48px rgba(19, 23, 32, 0.1), 0 4px 12px rgba(19, 23, 32, 0.05);
        ...
        {Array.from({ length: 11 }, (_, i) => {
          ...
          return (
            <div key={name} className="text-center">
              <div
                className={`border-border h-12 w-12 rounded-md border bg-${name}`}
              />
  ...
  render: () => (
    <div className="space-y-4 p-6">
      <h2 className="text-xl font-semibold">
        Verdict Family (platform-locked)
      </h2>
      <div className="space-y-2">
        {[
          ...
          {
            name: 'unk',
            label: 'Unknown / Not Recognised',
            severity: 'unknown',
          },
          {
            name: 'util',
            label: 'Utility (invalid, rate-limited, error)',
            severity: 'utility',
          },
        ].map(({ name, label }) => (
          <div key={name} className="flex items-center gap-3">
            ...
            <div
              className={`h-10 w-16 rounded bg-v-${name}-tint border border-v-${name}/20`}
            />
            <span className="text-fg text-sm font-medium">{label}</span>
            <span className="text-fg-muted font-mono text-xs">--v-{name}</span>
          </div>
        ))}
      </div>
    </div>
...
export const SpacingAndRadius: StoryObj = {
  render: () => (
    <div className="space-y-6 p-6">
      <div>
        <h2 className="mb-3 text-xl font-semibold">Spacing</h2>
        <div className="flex items-end gap-2">
          {[4, 8, 12, 16, 20, 24, 32, 40, 48, 64].map((s) => (
            <div key={s} className="text-center">
              <div className="bg-brand" style={{ width: s, height: s }} />
              <span className="text-fg-muted text-xs">{s}px</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex gap-3">
          {['xs', 'sm', 'md', 'lg', 'xl', 'full'].map((r) => (
            <div key={r} className="text-center">
              <div className={`bg-brand h-12 w-12 rounded-${r}`} />
              <span className="text-fg-muted text-xs">{r}</span>
            </div>
          ))}
        </div>
      </div>
...
function TenantThemeDemo({
  ...
  return (
    <div
      ref={ref}
      className="border-border bg-surface space-y-4 rounded-md border p-6"
    >
...
export default function Home() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-8">
      ...
      <p className="text-fg-muted mt-4 text-lg">Tenant console</p>
    </main>
  );
}
```
  ...
  return (
    <div className="relative">
      <button
        ...
        className="text-fg hover:bg-surface-sunken flex items-center gap-1.5 rounded-md px-2 py-1 text-sm font-medium transition-colors"
        aria-expanded={open}
          ...
          >
            {memberships.map((m) => (
              <li
                ...
                className={`hover:bg-surface-sunken cursor-pointer px-3 py-2 text-sm ${
              ...
              >
                {m.tenantName}
                <span className="text-fg-faint ml-2 text-xs">({m.role})</span>
              </li>
            ))}
          </ul>
        </>
...
export function UserMenu() {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="bg-brand text-brand-ink flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium"
        aria-expanded={open}
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
          <div className="border-border bg-surface absolute top-full right-0 z-50 mt-1 w-56 rounded-md border py-1 shadow-lg">
...
export default function DashboardPage() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="Dashboard"
        description="Overview of your product authenticity programme"
      />
      <EmptyState
  ...
  return (
    <div className="space-y-6">
      ...
      <EmptyState
        ...
        action={
          <a
            href="https://github.com/enendufrankc/verifynNG/issues/5"
            className="text-brand-text text-sm hover:underline"
          >
            View epic
          </a>
...
export function usePagedQuery<TItem>(
  ...
  fetcher: (
    cursor?: string,
  ) => Promise<{ items: TItem[]; nextCursor?: string }>,
) {
...
export default function TeamPage() {
  ...
  async function handleRemove() {
    ...
    setRemoveError('');
    try {
      await apiClient.delete(tenantPath(`/members/${removeTarget.id}`));
      queryClient.invalidateQueries({
        queryKey: queryKeys.team.list(activeTenantId ?? ''),
      });
            ...
            ? (member) => (
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="text-fg-muted hover:bg-surface-sunken rounded p-1"
                    title="Change role"
                  >
                    <Badge className="cursor-pointer text-xs">Edit role</Badge>
                  </button>
                  <button
                    onClick={() => setRemoveTarget(member)}
                    className="text-fg-muted hover:bg-surface-sunken hover:text-v-flag rounded p-1"
                    title="Remove member"
                  >
...
const STUB_SESSIONS = [
  ...
  },
  {
    ...
    lastSeenAt: '2026-01-18T09:00:00Z',
    current: false,
  },
];
...
export default function SecuritySettingsPage() {
  ...
  // MFA state
  const [mfaStep, setMfaStep] = useState<'idle' | 'setup' | 'verify' | 'codes'>(
    'idle',
  );
      ...
      <Tabs defaultValue="password">
        ...
        <TabsContent value="password" className="space-y-4 pt-4">
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            {passwordSuccess && (
              <p className="text-v-pos text-sm" role="status">
          ...
          {user?.mfaEnabled ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-center rounded-md bg-white p-4">
                ...
                <div className="bg-surface-sunken text-fg-muted flex h-48 w-48 items-center justify-center text-sm">
                  QR Code (E02)
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-v-pos-tint rounded-md p-4">
                <p className="text-v-pos text-sm font-medium">
                  Save your recovery codes
                </p>
                <p className="text-v-pos/80 mt-1 text-sm">
                  These codes can be used to sign in if you lose access to your
                  authenticator. Each code can only be used once.
                </p>
              </div>
              <div className="border-border bg-surface grid grid-cols-2 gap-2 rounded-md border p-4 font-mono text-sm">
        ...
        <TabsContent value="sessions" className="space-y-4 pt-4">
          <div className="space-y-3">
            {STUB_SESSIONS.map((session) => (
              <div
                key={session.id}
                className="border-border bg-surface flex items-center justify-between rounded-md border p-4"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-fg-muted text-xs">
                    IP: {session.ipPrefix}••• · Last active:{' '}
                ...
                {!session.current && (
                  ...
                  >
                    <Trash2Icon className="text-fg-muted h-4 w-4" />
                  </Button>
                )}
              </div>
            ))}
...
export default defineConfig({
  ...
  use: {
    baseURL:
      ...
      `http://localhost:${process.env.WEB_ADMIN_PORT || 4134}`,
