...
| `foundations/foundations-v1-broadsheet.dc.html` | **Direction B** — alternative canvas on slate/paper with Space Grotesk + IBM Plex, built from the tool's "Broadsheet" pack. |
...
## Status: direction not yet chosen

Two directions exist; E11 needs one. Decision pending (see the issue on E11). Whichever is chosen becomes `packages/ui` tokens; the other is deleted from this folder.
...
1. Choose Direction A or B.
2. Re-open the chosen canvas in Claude Design with the repo now populated so it can read `E06` and the E11 spec, fix the enum names, and proceed to the component inventory and key screens listed in `design-system-brief.md` §"Component inventory" and §"Key screens".
3. E11 imports the final tokens into `packages/ui` (Tailwind `@theme`) and treats this folder as the design source of truth.
